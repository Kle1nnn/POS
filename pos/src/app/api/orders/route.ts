...
type UpdateOrderBody = { orderCode: string; items: CartItemPayload[]; total: number; notes?: string; };
...
export async function PATCH(req: NextRequest) {
  let client;
  try {
    const { orderCode, items, total, notes } = (await req.json()) as UpdateOrderBody;
    ...
    const orderResult = await client.query(
      `SELECT id, status FROM orders WHERE order_code = $1 FOR UPDATE`, [orderCode]
    );
    ...
    if (status === "checkedout") { await client.query("ROLLBACK"); return NextResponse.json({ error: "Cannot edit a checked-out order" }, { status: 400 }); }

    await client.query(`UPDATE orders SET total = $1, notes = $2 WHERE id = $3`, [total, notes ?? null, orderId]);
...
export async function POST(req: NextRequest) {
  let client;
  try {
    const { orderCode: provided, items, total, notes, status, customerName, customerPhone, orderType, tableNumber } = (await req.json()) as SaveOrderBody;
    if (!items?.length) return NextResponse.json({ error: "Order must have at least one item" }, { status: 400 });
    ...
    const orderInsert = await client.query(
      `INSERT INTO orders (order_code, status, total, notes, customer_name, customer_phone, order_type, table_number,
         business_date, created_at, checked_out_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8,
         (SELECT current_business_date FROM store_state WHERE id = 1),
         NOW(), CASE WHEN $2 = 'checkedout' THEN NOW() ELSE NULL END)
       RETURNING id`,
