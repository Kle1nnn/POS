...
export default function Billing() {
  ...
  return (
    <div>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 right-0 mt-0 mr-0 rounded-xl h-screen bg-white text-black w-72 shadow-2xs
        ...
        <nav className="p-4 space-y-4">
          {navItems.map((item, index) => (
            <Link
              key={index}
              href={item.href}
              className="flex flex-col items-center justify-center w-18 aspect-square bg-white rounded-lg border border-white hover:bg-brown hover:text-white 
