...
export default function HistoryPage() {
  ...
  const fetchHistory = async (date: string, receipt?: string) => {
    const q = (receipt ?? "").trim();
    if (!q && !date) return;
    try {
      const params = new URLSearchParams();
      if (q) params.set("receipt", q);
      else params.set("businessDate", date);
  ...
  const handleReceiptSearch = async () => {
    ...
    await fetchHistory(businessDate, q);
  };
  ...
  const handleDelete = async (orderCode: string) => {
    ...
    try {
      ...
      const salesRes = await fetch("/api/sales");
      if (salesRes.ok) setSales(await salesRes.json());
      if (searchActive && receiptSearch.trim()) {
        await fetchHistory(businessDate, receiptSearch.trim());
      } else {
        await fetchHistory(businessDate);
      }
    } catch (e) {
  ...
  const handleRecalculate = async () => {
    if (
      !confirm(
        "Recalculate all sales totals from actual order data? This fixes any incorrect totals.",
      )
    )
      return;
    try {
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4 space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-gray-100">
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-[#1a3a5c] text-white text-sm font-semibold hover:bg-[#1565c0] transition-colors active:scale-95"
            >
              Search
            </button>
            {searchActive && (
              <button
                onClick={() => void clearReceiptSearch()}
                className="px-4 py-2 rounded-xl bg-gray-100 text-gray-700 text-sm font-semibold hover:bg-gray-200 transition-colors active:scale-95"
              >
                Clear
              </button>
            )}
                  ...
                  <div>
                    ...
                    {order.checkedOutAt && (
                      <p className="text-xs text-gray-400">
                        Checked out: {formatLocalDateTime(order.checkedOutAt)}
                      </p>
                    )}
                  </div>
