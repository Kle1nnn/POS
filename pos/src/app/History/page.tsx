...
type SalesSummary = {
  ...
  allTimeRevenue: number; allTimeOrders: number; allTimeItems: number;
};
...
export default function HistoryPage() {
  ...
  const openPrint = (order: any) => {
    const items: CartItem[] = (order.items || []).map((item: any) => ({
      ...item,
      cartKey: item.cartKey ?? `${item.name}-${item.selectedSize}-${item.selectedTopping}-${item.selectedSauce}`,
  ...
  const fetchHistory = async (date: string) => {
    if (!date) return;
    try {
      const res = await fetch(`/api/history?businessDate=${encodeURIComponent(date)}`);
      if (res.ok) {
  ...
  const loadStateAndSales = async () => {
    ...
    } catch (e) { console.error("Failed to load store state or sales", e); }
  };

  useEffect(() => { loadStateAndSales(); }, []);
  ...
  const handleDelete = async (orderCode: string) => {
    try {
      ...
      // Fetch sales and history sequentially AFTER delete commits — not in parallel
      const salesRes = await fetch("/api/sales");
      if (salesRes.ok) setSales(await salesRes.json());
      await fetchHistory(businessDate);
    } catch (e) {
  ...
  const handleRecalculate = async () => {
    ...
    try {
      const res = await fetch("/api/sales/recalculate", { method: "POST" });
      if (!res.ok) throw new Error("Failed");
      const salesRes = await fetch("/api/sales");
      if (salesRes.ok) setSales(await salesRes.json());
      alert("Sales totals recalculated successfully.");
    } catch (e) {
      alert("Recalculation failed. Check server logs.");
    }
  };
  ...
  const handleFixTotals = async () => {
    try {
      ...
      const salesRes = await fetch("/api/sales");
      if (salesRes.ok) setSales(await salesRes.json());
      alert("Sales totals recomputed from actual orders.");
    } catch (e) {
      alert("Failed to fix totals. Check server logs.");
    }
  };
      ...
      <div className="p-6 max-w-3xl">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4 space-y-3">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button onClick={handleFixTotals}
              ...
              title="Recompute totals from actual orders — use if numbers look wrong">
              🔧 Fix Totals
            </button>
            <button onClick={handleRecalculate}
              className="px-4 py-2 rounded-xl bg-red-50 text-red-700 border border-red-200 text-sm font-semibold hover:bg-red-100 transition-colors active:scale-95"
              title="Fix incorrect totals by recalculating from actual order data">
              🔄 Fix Totals
            </button>
            <span className="text-xs text-gray-400">Set this before opening each shift. Orders past midnight count to whichever date is active.</span>
          </div>
          {sales && (
            <div className="flex gap-4 text-sm flex-wrap pt-1 border-t border-gray-100">
              <span className="text-gray-500">Today's sales: <span className="font-bold text-gray-900">Rs. {sales.totalRevenue.toFixed(0)}</span></span>
              <span className="text-gray-400">|</span>
              <span className="text-gray-500">Orders: <span className="font-semibold text-gray-800">{sales.totalOrders}</span></span>
              <span className="text-gray-500">Items: <span className="font-semibold text-gray-800">{sales.totalItems}</span></span>
              <span className="text-gray-400">|</span>
              <span className="text-gray-500">All-time: <span className="font-bold text-gray-900">Rs. {sales.allTimeRevenue.toFixed(0)}</span></span>
            </div>
          )}
        </div>
        ...
        {orders.length === 0 ? (
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <div key={order.orderCode} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1 mb-1">
                      {order.orderType && (
                        <span className="text-[0.65rem] bg-amber-50 text-amber-800 font-semibold px-2 py-0.5 rounded-full border border-amber-100">
                          {order.orderType === "Dine In" && order.tableNumber ? `🍽️ Dine In · Table #${order.tableNumber}`
                    ...
                    <div key={i} className="flex justify-between items-center text-sm text-gray-700">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {item.selectedTopping && item.selectedTopping !== "None" && (
                          <span className="text-xs bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-full">{item.selectedTopping}</span>
                        )}
                        {item.selectedSauce && item.selectedSauce !== "None" && (
                          <span className="text-xs bg-yellow-100 text-yellow-600 px-1.5 py-0.5 rounded-full">{item.selectedSauce}</span>
                        )}
                      </div>
                ...
                <div className="border-t border-gray-100 pt-3 flex justify-between items-center">
                  <span className="font-bold text-gray-900">Total: Rs. {order.total.toFixed(0)}</span>
                  <div className="flex gap-2">
